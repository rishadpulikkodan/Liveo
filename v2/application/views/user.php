...
<body id="body" onload="snap();">
<script type="text/javascript">
  setInterval(function(){
    $.ajax("<?=base_url()?>index.php/user/rec/")
    .then(function(rec){
      $("#rec").html(rec);
    ...
    $.ajax("<?=base_url()?>index.php/user/nmsg/")
    .then(function(resp){
      if(resp=='1'){
  ...
  setInterval(function(){
    ...
    .then(function(mk){
      $("#img").attr("src","<?=base_url()?>res/img/guest"+mk+".jpg");
    });
  },1000);
  var ggps = '0.00, 0.00';
  setInterval(function(){
    gps();
    $.ajax("<?=base_url()?>index.php/user/getgps")
    .then(function(gps){
      if(ggps!=gps){
...
<script type="text/javascript">
  function sendm(){
    $.ajax("<?=base_url()?>index.php/user/sendm/")
    .then(function(resp){
  ...
  function msg(thi){
    ...
    $.ajax({
      url: "<?=base_url()?>index.php/user/msg/",
      type: "POST",
      data: { msg: thi } ,
      cache: false,
...
function snap(){
  ...
  setInterval( function(){
    ...
    xhr.open('POST', '<?=base_url()?>index.php/user/snap/', true);
    xhr.send(fd);
...
function gps(){
  if(navigator.geolocation){
    navigator.geolocation.getCurrentPosition(function(position){
      $.ajax("<?=base_url()?>index.php/user/pushgps/"+position.coords.latitude+"/"+position.coords.longitude);
    });
  }
  else{
    $.ajax("<?=base_url()?>index.php/user/pushgps/0.00/0.00");
  }
}
</script>
</body>
